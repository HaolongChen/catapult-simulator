...
export function AnimationControls({
  ...
  return (
    <div
      className={`fixed bottom-${UI_CONSTANTS.LAYOUT.OFFSET_STANDARD} left-1/2 -translate-x-1/2 bg-[#1e293b]/80 backdrop-blur-md p-${UI_CONSTANTS.LAYOUT.GAP_MEDIUM} rounded-2xl shadow-2xl border border-white/10 flex items-center gap-${UI_CONSTANTS.LAYOUT.GAP_LARGE} z-50 ring-1 ring-white/5`}
      style={{ width: UI_CONSTANTS.CONTROLS.WIDTH }}
    >
      <button
        onClick={onPlayPause}
        className={`w-${UI_CONSTANTS.CONTROLS.BUTTON_SIZE_MAIN} h-${UI_CONSTANTS.CONTROLS.BUTTON_SIZE_MAIN} rounded-full bg-blue-500 hover:bg-blue-600 text-white flex items-center justify-center transition-all hover:scale-105 active:scale-95 shadow-lg shadow-blue-500/20`}
      >
        {isPlaying ? (
          <Pause
      ...
      <button
        ...
        className={`w-${UI_CONSTANTS.CONTROLS.BUTTON_SIZE_SECONDARY} h-${UI_CONSTANTS.CONTROLS.BUTTON_SIZE_SECONDARY} rounded-full bg-white/10 hover:bg-white/20 text-slate-200 flex items-center justify-center transition-colors border border-white/10`}
      >
