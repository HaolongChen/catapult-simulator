...
for (let i = 0; i < 20; i++) {
  ...
  console.log('  Arm accel:', res.derivative.armAngularVelocity.toFixed(6), 'rad/s²')
  console.log('  Sling[0] pos:', state.slingParticles[0].toFixed(4), state.slingParticles[1].toFixed(4))
  console.log('  Sling[0] vel:', state.slingVelocities[0].toFixed(6), state.slingVelocities[1].toFixed(6))
  ...
  console.log('  Segment 0 length:', dist.toFixed(6), 'vs target:', Lseg.toFixed(6), 'error:', (dist - Lseg).toFixed(9))
