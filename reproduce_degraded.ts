...
for (let i = 0; i < 200; i++) {
  ...
  if (degraded && i > 0) {
    ...
    console.log(`  Arm angle: ${(state.armAngle * 180/Math.PI).toFixed(2)}°`)
    console.log(`  Arm velocity: ${state.armAngularVelocity.toFixed(4)} rad/s`)
    console.log(`  Proj pos: [${state.position[0].toFixed(3)}, ${state.position[1].toFixed(3)}]`)
    console.log(`  Proj vel: [${state.velocity[0].toFixed(3)}, ${state.velocity[1].toFixed(3)}]`)
    console.log(`  CW pos: [${state.cwPosition[0].toFixed(3)}, ${state.cwPosition[1].toFixed(3)}]`)
    break
  }
  
  if (i % 10 === 0) {
    console.log(`Frame ${i}, t=${state.time.toFixed(2)}s: ${degraded ? 'DEGRADED' : 'OK'}`)
  }
}
