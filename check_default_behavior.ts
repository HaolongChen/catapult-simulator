...
console.log('Initial ground normal:', computeDerivatives(state1, config1.projectile, config1.trebuchet, 100000).forces.groundNormal.toFixed(2), 'N')

for (let i = 0; i < 100; i++) {
  const res = computeDerivatives(state1, config1.projectile, config1.trebuchet, 100000)
...
console.log('Initial ground normal:', computeDerivatives(state2, config2.projectile, config2.trebuchet, 100000).forces.groundNormal.toFixed(2), 'N')

let liftedOff = false
for (let i = 0; i < 100; i++) {
  const res = computeDerivatives(state2, config2.projectile, config2.trebuchet, 100000)
...
if (!liftedOff) {
  ...
  console.log('This might be physically correct - insufficient CW mass to overcome ground friction')
}
